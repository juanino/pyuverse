...
Distributions for collector:
 Debian 6.0.4

== Screenshots
Included in the github depot are a few sample png's.
link:https://github.com/juanino/pyuverse/blob/master/samples/net.png
...
== Install
 cp ./bin/tools/empty.rrd [web_root]/net.rrd (you can also use mk_rrdtool.py provided)
 PYHTONPATH=[install_location]/bin/
 [install_location]/bin/inid_stats.py 
 Point your browser at the png files generated for stats viewing

== Simple Daemonizing
 PYHTONPATH=/root/appliance/inid/
 daemon /[install_location]/inid_stats.py \ 
    --stdout /var/log/inid_stats.log --stderr /var/log/inid_stats.log \
    --output /var/log/inid_stats.log

== Bugs
I've got quite a few items that need to go into a config file.
The code was originally written to write to /tmp so I need to parameterize all those spots.
